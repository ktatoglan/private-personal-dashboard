...
getName.innerHTML = "Unknown"
let getCity2 = "Ankara";
let getCity = JSON.parse(localStorage.getItem('userData'));
if (getCity) {
  getName.innerHTML = getCity[0].name
  ...
  if (this.readyState === this.DONE) {
    ...
    function prayerTime(prayerTimes) {
      for (let i = 0; i < prayerTimes.length; i++) {
        ...
        if (hours >= prayerTimes[5].hour || (hours === prayerTimes[5].hour && minutes >= prayerTimes[5].minute)) {
          ...
          prayerCountdown.innerHTML = `${remainingHours == 0 ? "in" : `in ${remainingHours} hours and`} ${remainingMinutes} minutes ${prayerTimes[0].vakit.toUpperCase()}`;
...
xhr.addEventListener("readystatechange", function () {
  if (this.readyState === this.DONE) {
    const data = JSON.parse(this.responseText).result
    let imdb_img = document.querySelector(".imdbID-img")
    
    if(data.Response === "False" || data.Type !== "movie") {
      xhr.open("GET", `https://api.collectapi.com/imdb/imdbSearchById?movieId=tt17351924`);
      xhr.setRequestHeader("content-type", "application/json");
      xhr.setRequestHeader("authorization", `${keys.colletcapi_key_m}`);
      xhr.send(data)
    }
    if (data.Poster === "N/A") {
      imdb_img.src = "https://placehold.co/600x400?text=Poster+NOT+Founded"
    } else {
      imdb_img.src = data.Poster
    }
      document.querySelector(".imdbID-title").innerHTML = `<strong>${data.Title}</strong>`
      document.querySelector(".imdbID-text").innerHTML = data.Plot
      document.querySelector(".imdbID-Director").innerHTML = `Director: ${data.Director}`
      document.querySelector(".imdbID-Genre").innerHTML = `Genre: ${data.Genre}`
      document.querySelector(".imdbID-Runtime").innerHTML = `Runtime: ${data.Runtime}`
...
function falseReturn() {
  var random7DigitNumber = generateRandom7DigitNumber();
...
function disa() {
  ...
  async function onSuccess(position) {
    ...
    const url = `https://api.opencagedata.com/geocode/v1/json?q=${lat}+${lng}&key=${keys.location_key}`;
    ...
    if (cty!=null){ 
      ...
      var city = data.results[0].components.state;
      var ilce = data.results[0].components.town;
    }
    ...
    function template() {
      fetch(
        `https://api.collectapi.com/health/dutyPharmacy?ilce=${ilce}&il=${city}`,
        {
          ...
          headers: {
            "Content-Type": "application/json",
            Authorization: keys.colletcapi_key_m,
          },
        }
      )
        .then((response) => {
